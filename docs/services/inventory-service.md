...
- **Read/Write persistence services, per the [persistence refactor](../refactoring/persistence-refactor-plan.md) (Phase 4 — the cross-aggregate-transaction reference case).** `Inventory.Application/Abstractions/Persistence/{Inventories,Warehouses,InventoryTransactions,StockDeductions}/` hold the four aggregates' Read/Write service ports; the four `I*Repository` interfaces are Persistence-internal now, trimmed to pure `Add`/bulk-`Delete` primitives (no query methods — those moved to each aggregate's `ReadService`, querying `InventoryDbContext` directly). All four repos are registered manually (none fit generic `IRepository<T>` any more — `WarehouseRepo` was the last one still Scrutor-scanned pre-migration, dropped along with the others).
- **`AdjustStock`/`DeductStock`/`RestockStock`/`StockIn`/`StockOut` are the migration's reference example for Correction 2** (cross-aggregate-root transactions — see the tracker). Every one of these commits `InventoryEntity` + `InventoryTransaction` (+ `StockDeduction` for Deduct/Restock) atomically, so their handlers keep injecting `IUnitOfWork` directly and call non-committing `Stage*` methods (`InventoryWriteService.StageUpdateAsync`, `InventoryTransactionWriteService.StageAddAsync`, `StockDeductionWriteService.StageAddAsync`/`StageUpdateAsync`) inside one `ExecuteTransactionAsync` the handler owns. Every other Inventory handler (`CreateWarehouse`, the 3 `OnProduct*` event handlers) is single-aggregate and uses the normal self-committing shape (`WriteService` methods that call `ExecuteTransactionAsync`/`SaveChangesAsync` themselves) — confirmed individually per handler, not assumed from the stock-mutation pattern.
