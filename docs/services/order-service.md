...
- **OrderProductCatalog** (`Order.Domain/Entities/OrderProductCatalog.cs`) — local read-model **keyed by `ProductVariationId`** (`Id` is the variation id itself, no surrogate key), carrying `ProductId`, `ProductName`, `Sku`, `Price`, `Status` (mirrors Product's `ProductVariationStatus` as of the last synced event; `IsOrderable` is `Status == "Active"`). Re-keyed from `ProductId` down to variation level because Product was redesigned so a single Product can have many priced `ProductVariation`s — a catalog row per product no longer has a single meaningful price. Kept in sync by five Product-originated integration event consumers (see "Messaging" below). Exists so `CreateOrderHandler` can validate/price/enable-check requested variations without a synchronous call to Product Service.
...
|---|---|---|
| `ProductVariationCreatedIntegrationEventConsumer` | `OnProductVariationCreatedEvent` | Upserts the catalog row keyed by `ProductVariationId` (create, or update Sku/Price/Status/ProductName if a redelivery races an update) |
| `ProductVariationUpdatedIntegrationEventConsumer` | `OnProductVariationUpdatedEvent` | Updates Sku/Price/Status on the existing row (logs a warning and skips if the row is somehow missing) |
| `ProductVariationDeletedIntegrationEventConsumer` | `OnProductVariationDeletedEvent` | Deletes the catalog row for that variation |
...
`ProductCreatedIntegrationEvent` (bare product creation, before any variation exists) has no consumer on the Order side — there's nothing useful to build a priced catalog row from until the first `ProductVariationCreatedIntegrationEvent` arrives, which happens in the same Product-side transaction anyway. `ProductVariationCreatedIntegrationEvent`/`ProductVariationUpdatedIntegrationEvent` gained a `Status` field (Product's `ProductVariationStatus`, e.g. `"Active"`/`"Inactive"`/`"Discontinued"`) specifically so Order's catalog can reject ordering a disabled variation — see `OrderProductCatalog.IsOrderable` above.
...
**Known naming mismatch:** `OrderItemCreateModel`/`CreateOrderCommand`'s `ProductId` field is looked up against `OrderProductCatalog` (keyed by `ProductVariationId`) unchanged — `CreateOrderHandler` still compiles and behaves correctly, but the field is semantically a ProductVariationId now, not a ProductId. Renaming `OrderItem`/`Order`'s own fields remains out of scope — flagging as a follow-up rename if Order's own commands are revisited.
...
- See the `ProductId`/`ProductVariationId` naming mismatch noted under "Messaging" above.
