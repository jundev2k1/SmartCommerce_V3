...
| `ProductVariationCreatedIntegrationEvent` (ProductId, ProductVariationId, Sku, ProductName, Price) | `CreateProductHandler` (once per initial variation) / `AddVariationHandler` | Inventory (creates a zero-stock row against the `MAIN` warehouse) + Order (creates an `OrderProductCatalog` row keyed by variation) + Product itself (Search sync) |
| `ProductVariationUpdatedIntegrationEvent` (ProductId, ProductVariationId, Sku, Price) | `UpdateVariationHandler` | Order (updates Sku/Price on the catalog row) + Product itself (Search sync) |
| `ProductVariationDeletedIntegrationEvent` (ProductId, ProductVariationId) | `DeleteVariationHandler` | Inventory (deletes Inventory rows for that variation) + Order (deletes the catalog row) + Product itself (Search sync) |
...
## Persistence: Read/Write services (Phase 5 of the migration)

Per the [persistence refactor](../refactoring/persistence-refactor-plan.md) — the reference case for a service with multiple *independent* aggregates and zero cross-aggregate write transactions (every `ExecuteTransactionAsync` call site here mutates exactly one repo; `categoryReadService`/`tagReadService` are only ever read before a transaction for validation, never written inside one). `Product.Application/Abstractions/Persistence/{Products,ProductCategories,ProductTags}/` hold the three aggregates' `I*ReadService`/`I*WriteService` ports; `I*Repository` interfaces are Persistence-internal now, trimmed to `AddAsync`/`Remove` only — no `Search`/`Exists`/`GetChildIds`/`GetAll`/tracked-`Update` methods, all of which moved to the matching `ReadService`/`WriteService` querying `ProductDbContext` directly. None of the three ever implemented generic `IRepository<T>` (custom query shapes, no conditional-include overload since `ProductVariation` is an EF owned type), so registration was already manual and stays that way.

`ProductWriteService.UpdateAsync` is self-committing and covers 9 of Product's 10 mutation handlers (details, variation update/delete/reorder/set-default, category/tag assign/remove) — their outbox events don't depend on the mutation's result, so the event is enqueued *before* calling `UpdateAsync` (Correction 1a: `EnqueueAsync` only stages a row, the Write Service's own `SaveChangesAsync` commits it together with the mutation). `AddVariation` is the one exception: its event needs the newly-generated variation's `Id`/`Sku`, which only exist *after* the mutation runs, so it uses the non-committing `StageUpdateAsync` instead and owns its own `IUnitOfWork.ExecuteTransactionAsync` wrapping both the mutation and the enqueue — a further generalization of Correction 2 (non-committing methods aren't only for cross-aggregate transactions; an output-dependent side effect needs the same shape).

## Persistence notes

- **`ProductVariation` is an EF owned collection** (`builder.OwnsMany(x => x.Variations, ...)`, table `product_variations`), same pattern as Order/OrderItem — owned rows are always loaded with the Product, no explicit `Include()`. `Variations`/`CategoryMappings`/`TagMappings` are plain `{ get; private set; }` auto-properties, so EF invokes the private setter directly — no `UsePropertyAccessMode(PropertyAccessMode.Field)` needed.
...
- **`ProductVariation.Images` is an EF Core primitive collection** (`ICollection<string>` mapped with `.HasColumnType("jsonb")` only — no explicit `ValueConverter`/`ValueComparer`, EF Core's built-in primitive-collection support handles the `jsonb` round-trip automatically).
- **Value Objects use a two-lambda `HasConversion`** (`x => x.Value, x => Sku.Create(x)`), and `Dimensions` (a compound VO) is mapped via nested `OwnsOne` inside the `product_variations` owned-type builder — `ComplexProperty` isn't available from an `OwnedNavigationBuilder` in this EF Core version.
- **Sku uniqueness is global**, not per-product — enforced both by a unique index on `product_variations.sku` and an app-level `IProductReadService.SkuExistsAsync` check (`SelectMany` over every product's owned `Variations`).
...
`ProductVariation`, `ProductCategory`, `ProductTag` have no such collision and are used by their plain names. [Inventory Service](inventory-service.md) has the identical issue for its `Inventory` entity — same fix, same reasoning.
...
- `CreateProductHandler` calls `uow.SaveChangesAsync(ct)` directly, twice, with no `ExecuteTransactionAsync` wrapper; `UpdateProductHandler`/variation update-and-delete handlers wrap the aggregate mutation in `ExecuteTransactionAsync` but then call the Outbox enqueue + a second, unwrapped `SaveChangesAsync` after it returns. Neither shape is fully atomic (a crash mid-workflow can commit the mutation but lose the event) and neither follows [04-coding-rules.md#transaction-management](../04-coding-rules.md#transaction-management) (added 2026-07-17, formalizing what Inventory's `StockInHandler`/`StockOutHandler`/`AdjustStockHandler` already do correctly). Not fixed here — a code change, out of scope for a docs pass.
