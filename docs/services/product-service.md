...
- **ProductCategory** (`Product.Domain/Entities/ProductCategory.cs`) — `Code` (`CategoryCode` VO, unique), `Name`, `Description`, `Status`, `ParentCategoryId` (self-referencing, nullable). Independent aggregate root, not owned by Product. No `Children` navigation — descendant traversal is a read-side query concern (`IProductCategoryRepository.GetChildIdsAsync`), not an in-memory Domain collection (a category tree can be arbitrarily deep; materializing it inside one aggregate instance would break the aggregate-per-transaction boundary). `ChangeParent` only guards direct self-parenting; deeper cycle detection (moving a category under its own descendant) is an Application-layer check (`UpdateProductCategoryHandler.EnsureNoCycleAsync`, walks the parent chain via repeated repository lookups) since it spans multiple aggregate instances.
...
## Repository shape: no longer implements generic `IRepository<T>`

`IProductRepository`/`IProductCategoryRepository`/`IProductTagRepository` are standalone interfaces (custom `Search`/`Exists`/`GetChildIds`/`GetAll` methods, no conditional-include overload now that `ProductVariation` is an EF owned type always loaded with its Product). None of the three match the generic `BuildingBlock.Persistence.Repository.IRepository<T>` shape anymore, so all three are registered manually in `Product.Persistence/DependencyInjection.cs` rather than picked up by the `AddScopedByInterface(typeof(IRepository<>), ...)` Scrutor scan — same reasoning as Inventory's `IInventoryTransactionRepository`/Audit's `IAuditLogRepository`.
...
- **Sku uniqueness is global**, not per-product — enforced both by a unique index on `product_variations.sku` and an app-level `IProductRepository.SkuExistsAsync` check (`SelectMany` over every product's owned `Variations`).
