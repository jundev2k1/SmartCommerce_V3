...
`RecordAuditLogHandler` maps the event's `AuditNode` tree (Contract) to `AuditTrailNode` (Domain) recursively, then is a pure sink: `repo.AddAsync(entry, ct)` then `uow.SaveChangesAsync(ct)` (the latter a documented no-op for Mongo, see below) — no Outbox enqueue.
...
- **`IAuditLogRepository` is not a generic `IRepository<T>`** — same reasoning as Inventory's `IInventoryTransactionRepository`: audit logs are append-only, and the only real query shape needed is a paged/filtered search (`SearchAsync`), not full CRUD. Registered explicitly in `Audit.Persistence/DependencyInjection.cs` rather than discovered by a Scrutor scan (there is no generic-interface scan to participate in here at all — Mongo services don't use the `AddScopedByInterface(typeof(IRepository<>), ...)` pattern).
