...
## The `/notifications` nav route is still a placeholder page, on purpose

Phase 1 reserved a full nav entry (`implemented: true`, unlike the four Campaign/Rule/Group/Channel placeholders) for `/notifications`, presumably anticipating a dedicated full-page history view. This phase's brief only asked for the Bell/Dropdown/Detail experience — no full-page notification list — so `(admin)/notifications/page.tsx` was left untouched (`PlaceholderModulePage`), rather than scope-creeping a page nobody asked for. A future phase could build a real full-history page here, reusing `shared/notifications`'s `NotificationList`/`NotificationItem` directly.
