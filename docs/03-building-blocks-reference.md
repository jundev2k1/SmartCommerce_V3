...
- **Persistence contracts** (`Abstractions/Persistence/`): `IRepository<T>`, `IUnitOfWork`
...
- **`BuildingBlock.Persistence` (contracts only — no ORM/database package reference, ever):**
  - `Repository/IRepository.cs` — generic repository abstraction
...
- **`BuildingBlock.Persistence.Ef` (the EF-aware implementation):**
  - `Repository/RepositoryBase.cs`, `UnitOfWork/EfUnitOfWork.cs` — EF implementations of the Application-layer `IRepository<T>`/`IUnitOfWork`
