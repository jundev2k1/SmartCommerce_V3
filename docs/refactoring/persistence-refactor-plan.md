...
- [ ] **Rework Phase 1 — User** to the Course Correction shape (repo regains full `IRepository<T>`, `UserProfileWriteService` methods become intent-named + non-self-committing, handlers own `IUnitOfWork` again, DI restores Scrutor scan).
- [ ] **Rework Phase 2 — Audit** to the Course Correction shape (Mongo — no `ExecuteTransactionAsync` involved, so impact is limited to repo regaining a full shape and `AuditLogWriteService.AddAsync` delegating through it; verify whether any change is even needed here beyond the repo/DI shape, since Audit's single bare-`SaveChangesAsync` method may already satisfy Correction rule 2 as long as it goes through a full repo, not direct DbContext use — Audit's `AuditLogRepo` never used direct DbContext-in-WriteService to begin with).
- [ ] **Rework Phase 3 — Auth** to the Course Correction shape (`AccountWriteService.DeleteIfExistAsync` currently self-commits via `ExecuteTransactionAsync` — must move to `OnAccountDeletionInitiatedHandler`; `RefreshTokenWriteService` was already non-committing under the old Correction-2 pattern, confirm it still matches the new uniform rule with no changes needed beyond the repo/DI shape).
- [ ] **Rework Phase 4 — Inventory** to the Course Correction shape (biggest rework after Product: `InventoryWriteService.AddAsync`/`DeleteByProductIdAsync`/`DeleteByVariationIdAsync` currently self-commit — move `ExecuteTransactionAsync` to `OnProductVariationCreatedHandler`/`OnProductDeletedHandler`/`OnProductVariationDeletedHandler`; `WarehouseWriteService.CreateAsync` bare-commits already matches Pattern A; the `StageUpdateAsync`-style methods across `Inventory`/`InventoryTransaction`/`StockDeduction` WriteServices were already non-committing under old Correction 2, confirm they still fit with no interface changes beyond repo/DI shape and intent-naming where still generic-delegate-shaped).
- [ ] **Phase 6 — Notification** (7 aggregates). Mechanically identical to Audit ×7 — no `ExecuteTransactionAsync` callers in this service at all. Build to the Course-Corrected shape from the start (full `IRepository<T>` repos, Scrutor scan, intent-named `WriteService` methods, no self-committing beyond bare `SaveChangesAsync`).
- [ ] **Phase 7 — Order** (`Order`+`OrderItem`, `OrderProductCatalog`). Independent Read/Write pairs for both, confirmed write-independent today. `Saga/` stays flat and untouched. `Features/Cart/**` stays out of scope. Build to the Course-Corrected shape from the start.
- [ ] **Phase 8 — Architecture standard.** Write `docs/conventions/persistence-coding-conventions.md` reflecting the Course-Corrected shape (not the original Corrections 1/1a/2/2a/2b), update `docs/02-architecture-rules.md`, `docs/03-building-blocks-reference.md` (note `IRepository<T>`'s extended shape), `docs/workflows/add-new-repository.md` (rewrite for the new pattern — repo still exists and still matters, just sits under a Read/Write service pair now), `docs/06-implementation-templates.md`'s repository template. Close out this checklist.
