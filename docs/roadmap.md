...
- **Actual outputs:** `features/auth` (`AuthGuard`/`GuestGuard`, login/register pages+forms, session query, logout) and `features/users` (create/edit against the real contract; list/delete placeholder-and-TODO since the User service doesn't expose them — see [modules/user-management.md](./modules/user-management.md) and [backend/coverage-report.md](./backend/coverage-report.md)). Two small additive touches to already-built shared files: `onLogout` callback prop threaded through `shared/layout/{AdminShell,Topbar,UserMenu}.tsx` ([decisions/0013](./decisions/0013-shared-shell-callback-props.md)), and `aria-invalid`/`aria-describedby` added to `shared/forms/FormField.tsx`.
...
- **Actual outputs:** `features/inventory` — Warehouses (Create + Detail real; Update/Delete disabled placeholders since neither endpoint exists; List resolved via browser-scoped local-id-tracking + manual "open by id," confirmed with the project owner given how much more limited this contract is than Order's), Stock (a product-scoped rollup lookup via `GetProductStock`, plus a locally-known inventory-record table), Stock Transactions (client-side aggregation of `GetInventoryHistory` across every locally-known inventory id — no "list all transactions" endpoint exists). New shared layer `shared/inventory/` (ADR 0016).
...
- **Completion criteria:** Inventory/warehouse/stock-transaction detail + write flows (stock-in/out/adjust) against the real contract — done; list views resolved via the documented local-tracking workaround rather than left descoped, since Create/GetById existing for both Warehouse and Inventory made a (browser-scoped) list a reasonable stopgap rather than a bare placeholder.
