...
- [x] Task 3 — Promotion Service Phase 2.5 Domain Standardization Review (`2026-08-07/Task3_promotion-service-phase2.5-domain-standardization-review.md`) — done. **Closes Phase 2 (Domain Model) — Phase 3 (Persistence) now current, 3/7.** Consolidated 7 Code VOs + 6 Period VOs + 4 Program-status enums into 3 shared types (also removed the `CouponCode` name-collision alias workaround), renamed `CampaignLocalization`→`CampaignTranslation`, added Translation support to 9 more aggregates, froze the Translation implementation workflow doc. First real `Promotion.Domain` build: 0 errors, 103 entities. **Domain model now frozen.**
