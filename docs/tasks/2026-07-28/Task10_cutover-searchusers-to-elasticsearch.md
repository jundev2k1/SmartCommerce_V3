...
- Frontend's exact current request shape (confirmed by the frontend agent's research, `SimpleShopUI/src/features/users/api/users.queries.ts:34-59`): `{ keyword, filters: [role filter, optional phone filter], page, pageSize }`, POSTed to `/api/user/users/search`. **If the endpoint's request/response contract changes at all** (even field renames), Frontend Task F4 must update in lockstep — coordinate the two.
