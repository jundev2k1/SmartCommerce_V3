...
- Integration tests (React Testing Library) for the highest-risk interaction flows: the refresh-token queuing behavior in `shared/lib/api/client.ts`, cart add/remove/quantity logic, the local-tracking stores (`local-orders`/`notifications`' `readIds` — Warehouse/Inventory no longer have local-tracking stores, see row below).
...
| Order has no list endpoint (Warehouse/Inventory resolved — `/warehouses/search`, `/inventories/search`, `/inventory-transactions/search` now exist and are wired in) | `shared/stores/local-orders.store.ts` — replace with a real paginated query once an Order list endpoint exists                                                                                         | [backend/coverage-report.md](./backend/coverage-report.md)           |
| User service has no delete endpoint (search + `Roles` resolved — `searchUsers` client + Admin/Normal-User tabs now wired in)                                         | `features/users` — revisit once `DELETE /profiles/{userId}` exists                                                                                                                                     | [modules/user-management.md](./modules/user-management.md)           |
