...
4. Even with Inbox dedup, make the target command handler idempotent if a duplicate side effect would be wrong (e.g. creating a second row) — see `User.Application`'s `CreateUserCommandHandler` check-then-create-then-recheck pattern, or Inventory's `GetByVariationAndWarehouseAsync` pre-check in `OnProductVariationCreatedEvent`/`Handler`. Inbox dedup protects against *redelivery of the same message*; it doesn't protect against two *different* messages producing the same logical effect.
