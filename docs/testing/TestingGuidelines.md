...
**Scope:** how to write an individual test in this repo — structure, naming, mocking rules, when to reach for `SimpleShop.TestKit`. Read [TestingArchitecture.md](TestingArchitecture.md) first if you haven't seen the `/tests` layout yet.
...
- `SimpleShop.TestKit` fakes' own callers — the fakes (`FakeCurrentUserService`, `FakeAppLogger<T>`) exist precisely so you don't need a mocking framework for these; use them directly, don't wrap them in a mock too.
...
Reach for a `TestDataBuilder<T>` subclass (see `SimpleShop.TestKit.Builders`) when a type's `Create` factory takes more than ~4 arguments and most tests only care about overriding one or two of them (e.g. an aggregate like `Product` or `NotificationCampaign`). For a single-argument Value Object factory (`Sku.Create(value)`), just call it directly — a builder would add indirection with no readability win.
...
- **New reusable `BuildingBlock.*` infrastructure** → unit tests belong in the matching `BuildingBlock.*.Tests` project, using `SimpleShop.TestKit` where applicable.
