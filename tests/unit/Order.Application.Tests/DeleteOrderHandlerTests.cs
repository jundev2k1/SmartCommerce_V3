...
public sealed class DeleteOrderHandlerTests
{
    ...
    public async Task Handle_RestocksInventory_BeforeDeleting(bool cancelled)
    {
        ...
        var handler = new DeleteOrderHandler(repo, outbox, inventory, uow);
        ...
        await repo.Received(1).DeleteAsync(order.Id, Arg.Any<CancellationToken>());
        ...
        Received.InOrder(() =>
        {
            ...
            repo.DeleteAsync(order.Id, Arg.Any<CancellationToken>());
        });
    }
