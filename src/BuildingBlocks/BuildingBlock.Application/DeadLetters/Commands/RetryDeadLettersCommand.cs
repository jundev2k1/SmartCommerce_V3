...
public sealed record RetryDeadLettersCommand(IReadOnlyList<Guid> Ids) : ICommand<RetryDeadLettersSummary>;
...
{
    public async Task<RetryDeadLettersSummary> Handle(RetryDeadLettersCommand request, CancellationToken ct = default)
    {
        var succeeded = 0;
        var failed = 0;
        ...
        foreach (var id in request.Ids)
        {
            ...
            {
                failed++;
                skipped.Add(new RetryDeadLettersSkip(id, result.Outcome.ToString()));
            }
        }

        return new RetryDeadLettersSummary(request.Ids.Count, succeeded, failed, skipped);
    }
}
