    ...
    IConfiguration configuration)
{
    private readonly TimeSpan _defaultTtl = TimeSpan.FromMinutes(
        configuration
            ...
            .Get<int?>() ?? CacheKeys.Roles.DefaultTtlMinutes);
    ...
    {
        var keys = userIds.Select(id => CacheKeys.Roles.UserRoles(id)).ToList();
        if (keys.Count > 0)
    ...
    {
        await cacheService.RemoveByPatternAsync(CacheKeys.Roles.UserRolesPattern, ct);
    }
