...
import { useLocalWarehousesQuery } from '../api/inventory.queries';
...
export function WarehousesListPage() {
  ...
  const { warehouses, isLoading, hasAny } = useLocalWarehousesQuery();
  ...
  const filtered = useMemo(() => {
    ...
    return warehouses.filter(
      (w) => w.name?.toLowerCase().includes(query) || w.code?.toLowerCase().includes(query),
    );
  }, [warehouses, search]);
